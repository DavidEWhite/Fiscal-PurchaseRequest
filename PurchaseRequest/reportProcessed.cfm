...
SELECT service, recordID, rName, fcpgpf, fbPurpose, activity, amount, aDate, fDate, dDate, dtrigger
FROM singlePurchase
WHERE (((dTrigger=1) OR (dTrigger=2) OR (aTrigger=2) OR (fTrigger=2)) 
AND (rDate BETWEEN ###FORM.bDate### AND ###FORM.eDate###))
ORDER BY Service, recordID DESC
</cfquery>
<!--- blanket purchase --->
<cfquery name="get_list2" datasource="FiscalPurchase">
SELECT service, recordID, rName, fcpgpf, fbPurpose, activity, amount, aDate, fDate, dtrigger
...
<TD valign="top" class="mini">#DateFormat(aDate , "mm/dd/yy")#</TD>
<TD valign="top" class="mini">#DateFormat(fDate , "mm/dd/yy")#</TD>
<TD valign="top" class="mini">
<CFIF #dTrigger# EQ "1">Approved</CFIF><CFIF #dTrigger# EQ "2">Disapproved</CFIF></TD>
</TR>
</CFOUTPUT>
</TABLE>
</CFIF>
